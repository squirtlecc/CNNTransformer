...
def load_culane_data(data_dir, file_list_path):
    with open(file_list_path, 'r') as file_list:
        filepaths = [
            os.path.join(data_dir, line.split()[0][1 if line[0] == '/' else 0:].rstrip().replace('.jpg', '.lines.txt'))
...
def eval_predictions(
        pred_dir, anno_dir, list_path, width=30,
        img_shape=(590, 1640, 3), official=True, sequential=False):
