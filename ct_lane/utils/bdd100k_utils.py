...
if __name__ == "__main__":
    ...
    mask_path = '/disk/lane-datasets/BDD100K/seg_label/lane_gt/train/ab5325cd-913693d3.png'
    mask_path_2 = '/disk/lane-datasets/BDD100K/seg_label/lane_gt/train/0a0c3694-4cc8b0e3.png'
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    seg = torch.from_numpy(mask).long()
    seg[seg == 255] = 1
    seg = seg.unsqueeze(0)
    # seg = F.one_hot(seg.long(), num_classes=seg.max().item() + 1)
    mask_2 = cv2.imread(mask_path_2, cv2.IMREAD_GRAYSCALE)
    seg2 = torch.from_numpy(mask_2).long()
    seg2[seg2 == 255] = 1
    seg2 = seg2.unsqueeze(0)
    seg_annot = seg2
    # seg_annot = seg.clone()
    print(seg.shape, seg_annot.shape)
    print(seg.unique(return_counts=True), seg_annot.unique(return_counts=True))
    tp_seg, fp_seg, fn_seg, tn_seg = get_stats(seg.cuda(), seg_annot.cuda(), mode='multiclass', threshold=None, num_classes=2)
    # tp_seg = tp_seg[0][1]
    iou = iou_score(tp_seg, fp_seg, fn_seg, tn_seg, reduction='none')
    # f1 = balanced_accuracy(tp_seg, fp_seg, fn_seg, tn_seg, reduction='none')
    f1 = f1_score(tp_seg, fp_seg, fn_seg, tn_seg, reduction='none')

    iou_ls = iou.detach().cpu().numpy()
    f1_ls = f1.detach().cpu().numpy()
    print(tp_seg.detach().cpu().numpy(), fp_seg.detach().cpu().numpy(), fn_seg.detach().cpu().numpy(), tn_seg.detach().cpu().numpy())
    print(iou.shape, np.unique(iou.cpu(), return_counts=True))
    print(f1.shape, np.unique(f1.cpu(), return_counts=True))
    iou_score = np.mean(iou_ls)
    # print(iou_score)
    f1_score = f1
    # [micro, macro, weighted, micro-imagewise, macro-imagesize, weighted-imagewise, none, None]
    print(tp_seg.sum(), fp_seg.sum(), fn_seg.sum(), tn_seg.sum())
    accuracy = balanced_accuracy(tp_seg, fp_seg, fn_seg, tn_seg, reduction="none")
    recall = recall(tp_seg, fp_seg, fn_seg, tn_seg, reduction="none")
    print(f"f1:{f1_score}, accuracy:{accuracy}, recall:{recall}")