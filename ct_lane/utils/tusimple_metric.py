...
class LaneEval(object):
    ...
    def bench(pred, gt, y_samples, running_time):
        ...
        for x_gts, thresh in zip(gt, threshs):
            accs = [LaneEval.line_accuracy(
                    np.array(x_preds),
                    np.array(x_gts), thresh) for x_preds in pred]
        ...
        # fp = fp / len(pred) if len(pred) > 0 else 0.
        # fn = fn / max(min(len(gt), 4.), 1.)
        # return acc, fp, fn
    ...
    def bench_one_submit(pred_file, gt_file):
        try:
            ...
            raise Exception(f"Fail to load json file of the prediction.\n{e}")
        ...
        for pred in json_pred:
            ...
            try:
                ...
                raise Exception(f"Format of lanes error.\n{e}")
            accuracy += a
            fp += p
            fn += n
        num = len(gts)
