...
# TODO: change this models
from shapely.geometry import LineString, Polygon

from . import llamas_utils
...
def load_labels(label_dir):
    ...
    annos = [
        [
            add_ys(xs) for xs in llamas_utils.get_horizontal_values_for_four_lanes(label_path)
...
def eval_predictions(pred_dir, anno_dir, width=30, unofficial=False):
    ...
    results = []
    # loop = tqdm(map(partial(
    #     culane_metric, width=width,
    #     unofficial=unofficial, img_shape=LLAMAS_IMG_RES),
    #     predictions, annotations), total=len(predictions))
    # for r in loop:
    #     results.append(r)

    anno_files = llamas_utils.get_files_from_folder(anno_dir, "json")
    # TODO: need optimizate get all files path from folder
    label_paths = [llamas_utils.get_label_base(p) for p in anno_files]
    pred_files = [os.path.join(pred_dir, path.replace('.json', '.lines.txt')) for path in label_paths]
    loop = tqdm(zip(anno_files, pred_files), total=len(anno_files))
    for i, (anno, pred) in enumerate(loop):
        gt_label = [
            add_ys(xs) for xs in llamas_utils.get_horizontal_values_for_four_lanes(anno)
            if (np.array(xs) >= 0).sum() > 1]  # lanes annotated with a single point are ignored

        with open(pred, 'r') as pred_file:
            pred_label = pred_file.readlines()
        pred_label = [line.split() for line in pred_label]
        pred_label = [list(map(float, lane)) for lane in pred_label]
        pred_label = [[(lane[i], lane[i + 1]) for i in range(0, len(lane), 2)] for lane in pred_label]
        pred_label = [lane for lane in pred_label if len(lane) >= 2]
        result = culane_metric(pred_label, gt_label, width=width, unofficial=unofficial, img_shape=LLAMAS_IMG_RES)
        results.append(result)

    total_tp, total_fp, total_fn = np.array(results).sum(axis=0)
    ...
    result = {
        ...
        'FN': total_fn, 'Precision': precision,
        'Recall': recall, 'F1': f1}
    return result
